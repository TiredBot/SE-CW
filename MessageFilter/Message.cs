...
namespace MessageFilter
{
    ...
    /*
     EMAIL REGEX*/
       ...
       {
           public static List<Email> EmailsReceived = new List<Email>();
           public static List<Email> SIRReceived = new List<Email>();
           ...
           private static Regex SIRsubject = new Regex(@"^SIR [0-9]{2}\/[0-9]{2}\/[0-9]{4}\b");
           //SIR REGEX for subject incedient report and body(\d{2}\-\d{3}\-\d{2})\n(.*)\n(.{1,1028})
           public bool SIRcheck()
           {
               bool SIRfound = SIRsubject.IsMatch(this.Subject);
               if(SIRfound)
               {
                   SIR = true;
                   return true;
           ...
           {//Fix Regex to split the message into its parts as Groups
               Match SIRTemp = Email.EmailProcess.Match(this.MessageBody);
               ...
               this.MessageText = SIRTemp.Groups[15].Value;
           }
